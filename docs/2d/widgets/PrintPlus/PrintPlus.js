define([
  ...
  var PrintDijit = declare([_WidgetBase, _TemplatedMixin, _WidgetsInTemplateMixin], {
    ...
    postCreate: function() {
      ...
      if (this.showLayout) {
        this.mapUnitsToMeters = this.getUnitToMetersFactor(this.map._params.units);  //lcs - this may break in future API releases
        if (isNaN(this.mapUnitsToMeters.x) || isNaN(this.mapUnitsToMeters.y)) {
          ...
          // lp.lengthUnit = esriConfig.defaults.geometryService.UNIT_METER;  // This is undefined.  Don't know why.
      ...
      if (this.showLayout) {
        ...
        this.printGL.on('click', lang.hitch(this, function(evt) {
    ...
    _onOpen: function() {
      if (this.printGL) {
        ...
        this.toggleGraphicsLayer(true);
      }
    },
    
    _onClose: function() {
      this.isDocked = false;
      if (this.printGL && !this.showLayoutDijit.get('value')) {
        this.toggleGraphicsLayer(false);
      }
    },
    ...
    printDefInspector: function(printDef) {
      ...
      } else if (this.printSettingsFormDijit.get('value').layout === 'MAP_ONLY' && this.preserve.preserveScale === 'false') {
        console.log(1, 'printDef.mapOptions:', printDef.mapOptions);
        printDef.mapOptions.scale = this.mapOnlyScale;
        console.log(2, 'printDef.mapOptions:', printDef.mapOptions);
      }
      return printDef;
    },
    ...
    toggleMapPanHandlers: function(turnOn) {
      if (this.printGL && this.printGL.visible && this.showLayoutDijit.get('value') && turnOn) {
        var _handler;
        ...
        this.dragSwipeHandlers.push(_handler);
        _handler = this.map.on('pan-end', lang.hitch(this, function(evt) {  //'swipe-end'
          if (this.mapSheetMoving) {
            this.mapSheetMoving = false;
    ...
    drawMapSheet: function(centerPt) {
      ...
      var closeBtnSize = Math.min(14, Math.round(layoutMargin / mapUnitsToPixels) - 2);
      ...
      this.printGL.add(graInstructionBackground);
      this.printGL.add(graInstruction);
    },
    ...
    moveMapSheet: function(mOffset) {
      ...
      array.forEach(this.printGL.graphics, lang.hitch(this, function(gra) {
    ...
    onStateChange: function(reason, newValue) {
      ...
      switch (reason) {
        case 'TITLE_BLOCK':
          ...
          registry.byId('titleInput').set('disabled', !newValue);
        ...
        case 'LAYOUT':
          ...
          registry.byId('titleInput').set('disabled', newValue === 'MAP_ONLY');
    ...
    _toggleShowLayout: function(show, layoutClicked) {
      ...
      if (show) {
        ...
        if (layoutClicked) {
          ...
          // turn off printGL if the widget is closed
    ...
    toggleGraphicsLayer: function(visible) {
      this.printGL.setVisibility(visible);
